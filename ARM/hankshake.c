...
void process_packet(unsigned char *buffer, int size)
{
    ...
    if (iph->protocol == 6)
    {
        ...
        if (tcph->syn == 1 && tcph->ack == 1 && source.sin_addr.s_addr == dest_ip.s_addr)
        {
            ...
            // Send an ACK packet in response to SYN-ACK
            send_ack_packet(source, dest, ntohs(tcph->dest), ntohs(tcph->source));
        }
    }
}

void send_ack_packet(struct sockaddr_in source, struct sockaddr_in dest, int source_port, int dest_port)
{
	perror("Send ack package====");
    int s = socket(AF_INET, SOCK_RAW, IPPROTO_TCP);
    if (s < 0)
    {
	...
	iph->protocol = IPPROTO_TCP;
	iph->check = 0;		//Set to 0 before calculating checksum
	iph->saddr = inet_addr ( "192.168.1.6" );	//Spoof the source ip address
	iph->daddr = dest_ip.s_addr;
    ...
    // Set destination address for ACK
    ack_dest.sin_family = AF_INET;
    ack_dest.sin_addr.s_addr = source.sin_addr.s_addr;
    ...
    // Send the ACK packet
    if (sendto(s, datagram, sizeof(struct iphdr) + sizeof(struct tcphdr), 0, (struct sockaddr *)&ack_dest, sizeof(ack_dest)) < 0)
    {
	...
	{
		perror("Send ack package====xx success");
	}
